...
(() => {
  ...
  const CONFIG = {
    ...
    passStructuredObjectsToReact: false,
  ...
  function matches(url, patterns) { return patterns.some((pattern) => pattern.test(url || "")); }
  ...
  function trimTurns() { while (transcript.turns.length > CONFIG.maxTurns) transcript.turns.shift(); }
  ...
  function addUserText(text) {
    ...
    if (last && !last.assistant && last.user === text) { transcript.current = last; return; }
  ...
  function updateBadge(text) {
    if (!CONFIG.showBadge) { page.document?.getElementById(BADGE_ID)?.remove(); return; }
    installBadge();
  ...
  function scheduleBadge() {
    ...
    badgeTimer = page.setTimeout(() => { badgeTimer = 0; updateBadge(); }, 250);
  }
  ...
  function installPanel() {
    ...
    clear.addEventListener("click", () => { transcript.turns = []; transcript.current = null; stats.lastText = ""; stats.lastStableText = ""; updatePanelNow(); });
    const hide = page.document.createElement("button");
    hide.textContent = "Hide";
    hide.style.cssText = "font-size:12px;padding:3px 7px;";
    hide.addEventListener("click", () => { CONFIG.showPanel = false; panel.remove(); });
  ...
  function normalizePath(path) { return Array.isArray(path) ? "/" + path.join("/") : String(path || ""); }
  function isContentPath(path) { return /\/message\/content\/parts(?:\/|$)|\/content\/parts(?:\/|$)|\/text(?:\/|$)|\/body(?:\/|$)|\/output(?:\/|$)/i.test(normalizePath(path)); }
  function rememberPath(path) { const normalized = normalizePath(path); if (isContentPath(normalized)) streamState.currentContentPath = normalized; }
  function isPatchObject(json) { return Boolean(json && typeof json === "object" && typeof json.o === "string" && ("p" in json || "v" in json)); }
  ...
  function processPayload(kind, data) {
    let json = null, text = "", passToReact = false, reason = "blocked";
    if (data === "[DONE]") {
      finishTurn();
    ...
    if (!json || typeof json !== "object") {
      if (kind === "delta_encoding") { passToReact = true; reason = "delta-encoding-control"; }
      else { stats.malformedEventsBlocked++; passToReact = false; reason = "non-object-blocked"; }
  ...
  function processSSE(raw) { if (!raw.trim()) return { passToReact: false }; stats.events++; const parsed = parseSSE(raw); return processPayload(parsed.event, parsed.data); }
  function processLine(raw) { const line = raw.trim(); if (!line) return { passToReact: false }; stats.events++; return processPayload("ndjson", line); }
      ...
      try {
        while (!closed) {
          ...
          if (done) {
            ...
            if (tail) { capture(tail); pending += tail; }
            flush(true); closed = true; try { controllerRef.close(); } catch {}; stats.transformed++; updateBadge(); updatePanelNow(); return;
          }
          if (!value) continue;
          stats.bytes += value.byteLength || value.length || 0;
          const text = decoder.decode(value, { stream: true });
          capture(text); pending += text; flush(false); scheduleBadge();
        }
      } catch (error) { closed = true; stats.failed++; stats.lastError = String(error?.message || error); try { controllerRef.error(error); } catch {} }
    }
    function flush(final) {
      if (isEventStream) {
        const events = pending.split("\n\n"); pending = final ? "" : (events.pop() || "");
        for (const raw of events) emit(raw, "sse"); if (final && pending.trim()) emit(pending, "sse");
      } else {
        const lines = pending.split(/\r?\n/); pending = final ? "" : (lines.pop() || "");
        for (const raw of lines) emit(raw, "line"); if (final && pending.trim()) emit(pending, "line");
      }
    }
    function emit(raw, kind) {
      const result = kind === "sse" ? processSSE(raw) : processLine(raw);
      if (result.passToReact && !closed) { stats.controlEventsPassed++; try { controllerRef.enqueue(encoder.encode(raw + (kind === "sse" ? "\n\n" : "\n"))); } catch {} }
      else stats.eventsSwallowed++;
    }
  ...
  page.fetch = async function patchedFetch(input, init) {
    ...
    if (!shouldSink(url, response)) { stats.passed++; scheduleBadge(); return response; }
    return makeTransformedResponse(response, contentType);
  };

  page.cgptRenderSink = {
    config: CONFIG, stats, transcript,
    ...
    clearCapture() { stats.lastCapture = ""; stats.lastText = ""; stats.lastStableText = ""; stats.lastJSON = null; stats.parsed = []; transcript.turns = []; transcript.current = null; streamState.currentContentPath = ""; updatePanelNow(); },
  };
